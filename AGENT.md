...
本项目是一个 Go 重写版：原版是 `old_code/` 下的 Python + Textual 智能体运行时，功能已基本对齐，
当前工作重点是 TUI 的还原度、发布流程和稳定性。这个文件写给在这个工作区里干活的 AI 助手，
只写真正影响判断的东西。

`old_code/` 是**参考材料，不是源码**：可以读、可以对照，不要改，也不要照着它的结构重写 Go 代码。
