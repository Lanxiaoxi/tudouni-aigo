...
**`code == "auth"` 是受管 token 的消息**（`--ericai`），两种时机各有各的出处：
**开局那条在 `init.notices` 里**（"这个会话带着哪支 token 开的"），**跑着跑着的那条在这条
消息里**。后者**只在回合结束时到**：runtime 把它排在队列里，回合收尾时一并交出 ——
`DrainAuthNotices` 是个**可选接口**，不认识 token 的 runtime 不实现它，协议层也不必知道
token 是什么（和 goal 那几个钩子同一种做法）。

为什么必须等到回合结束：这条消息说的是**那一轮**发生的事（"token 换了，请求重发了一次"），
回合还在跑的时候发出去，它会落在**正在画的**那一轮下面，而前端没有回头重排的能力。
`level` 是 `info`（换好了）或 `warn`（**没换成，旧 token 还在那儿**）—— 这一条要能一眼分出
"试过了但不行"和"压根没试"，因为这两件事的下一步不一样。
...
**`effort` 认的是"当前这个模型声明的那些档"**，也就是同一份 `init.effort_levels`
（见下）。模型没声明时是**一整套** `minimal` / `low` / `medium` / `high` / `xhigh`
/ `max`。**不做折算** —— 以前有一张折叠表（`xhigh→high`、`ultra→max` …），它同时
改掉了发出去的报文和用户看到的字，于是屏幕上写着 `xhigh`、账单上是 `high`。现在打
一个当前模型不收的档就回一条 `notice` 报错，**什么都不改**。列宽是有意的：选错最多
换来端点一次可见的 400，列窄则是"一个能用的档你永远选不到"，而后者没人会发现。
**`"none"` 不是一档强度**：它是"关掉思考"，走 `set_thinking`。
...
一档"。**清单是"当前这个模型"的属性，不是全局常量**：路由或模型条目可以在配置里声明
`effort_levels`（不声明就用那一整套）。切换模型之后它跟着变，而 `/effort` 的面板里那份
是打开那一刻的快照。
...
| **凭证被拒（401/403），且 runtime 管着这支 token** | 自己换一支再发**一次**；成功则这一轮正常结束，另加一条 `notice(code="auth")`。换不到就照上面那条报 | 什么都不用做。**看到 `code="auth"` 别当成错误**：它说的是"这一轮中途换过一次凭证" |
...
7. **有哪些档强度、哪个模型在哪条路由上**（3.11 / 3.9）—— 两者都随协议发
   ...
   "清单里列着它，打进去说没有这一档"。**更别自己把档位折算了**：`effort_levels`
   是"当前这个模型收哪些"，打一个不在里面的就该报错，而不是替用户发一个别的档。
