...
（v2.7 把这条规则推广了：任务列表 / goal / **后台任务**三种各有自己的边沿，但 MCP 仍然不
在里面 —— 见 21.4。用户起的后台命令要顶开，是因为他在等它；MCP 是配置。）
...
| 谁产生 | 例子 | 在哪儿翻 |
|---|---|---|
| TUI 父进程 | 三条栏、左栏五块、回合头、命令面板、欢迎屏、五个弹层、状态栏那一行的 `activity` | `frontends/tui/*`（查目录表） |
...
## 二十一、v2.7：左栏重塑（2026-09）

真实使用一轮之后按用户的要求改的：**删两块、限一块、加宽、三种东西都能顶开**。
判据还是决策 26 那条 —— 「一眼能看出它理解得对不对」比省那几列要紧 —— 只是把
"值得顶开"的清单从一项扩到三项。

### 21.1 删掉 Session 块（它比其余所有块加起来还高）

那一块里的每一条**在别处都有更该读它的地方**：

| 原来在 Session 块里 | 现在在哪儿 |
|---|---|
| session id | 顶栏 |
| model + 窗口 | 会话栏 |
| `12 messages · 4 steps` | 状态栏右侧（`status.session.*`） |
| `context 22.5k / 128.0k (17.6%)` | 状态栏右侧 |
| audit 路径 | 状态栏右侧 |
| **thinking off / effort** | **effort 挪到会话栏、模型名后面的括号里**（`glm-5.3 (high)`）；thinking 关着这件事只在 `/thinking` 里说 |
| **AGENT.md 加载情况** | **不画了** —— 启动那条 notice 已经说了，而且这个问题是启动时问的，不是随时问的 |

代价说清楚：**effort 现在无条件显示**（以前只在 thinking 关着时显示）。理由是这个值两种
情况下都真的生效，一个跟着另一个开关忽隐忽现的值，没人会信。窄屏（<120 列）会话栏本来
就不显示模型，括号跟着一起不显示。

### 21.2 objective 限行，块也限行

一条 objective 是模型写的一段话，而它之前能吃掉 23 行里的一半 —— 下面三块全被挤出屏幕。
现在：

- **Goal 的 objective 最多 6 行**，第 7 行写 ``… (+N more lines) — `/goal` ``：截断这件事
  必须自己说出来，否则"被截断的 objective"读起来就是"目标的全部"；
- **每块还有一个硬上限**（Tasks / Background jobs 也是 6 行），超出的行数并进最后一行
  `(+N more)`。这两块**没有内容上限** —— 用户要的是"我起的每一个任务和后台命令都看得见"
  —— 这个上限只在矮终端上才咬人；
- 装不下的块仍然从**下往上**丢（Goal 在最上面，永远活到最后）。

### 21.3 加宽：从常量变成随终端缩放

`railWidth = 32` 变成 `clamp(width/3 - 4, 32, 48)`，**会话流保底 48 列**。收益直接体现在
折行上：同一个 objective 在 30 格内容宽下要 9 行，在 38 格下 6 行，在 46 格下 4 行。
80 列的终端 -> 32；120 -> 36；150 -> 46；200 以上封顶 48。

### 21.4 三种东西都能顶开它（决策 26 的推广）

原来只有任务列表的边沿能顶开。现在 **任务列表 / goal / 后台任务**三种都是「从无到有顶开
一次，回到空重新武装」（`noteRail`），理由是同一个：这三样都是"它现在在干什么"里，
用户唯一能提前看出"理解得对不对 / 我那条命令还在不在跑"的地方。

- `/resume` 回来的会话**也会顶开一次**（快照里已经带着 goal 和任务列表）—— 刚回到一个
  自己记不清的会话时，这正是最需要看的一眼；
- **`railPinned` 这个字段删了**。它从决策 26 落地起就只写不读（M2 的遗留），留着一个
  名字在撒谎的字段比没有更糟；
- 手动 `Ctrl+B` 收起之后**保持收起**：边沿只在"新东西出现"时触发，内容更新（一条任务
  完成、一个 job 结束）不触发。

### 21.5 块的顺序：Background jobs 上移到 MCP 前面

Goal · Tasks · Loaded skills · **Background jobs** · MCP。

理由：后台任务和 MCP 都是"这台机器上活着的东西"，但**后台任务是用户自己按的、正在等的**
（而且它是唯一显示 `result not collected` 的地方），MCP 是背景设施。两者都在 Session 块
被删掉之后才敢这么排 —— 上移的代价本来是"Session 被挤下去"，现在没有 Session 了。
